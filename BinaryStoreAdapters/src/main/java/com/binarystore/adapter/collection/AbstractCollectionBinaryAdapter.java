...
public abstract class AbstractCollectionBinaryAdapter<T extends Collection> extends AbstractBinaryAdapter<T> {
    ...
    @CheckForNull
    private Object deserializeElement(@Nonnull ByteBuffer byteBuffer, @Nonnull Adapters adapters) throws Exception {
    ...
    private void checkSubCollectionBounds(int startIndex, int endIndex, int collectionSize) throws RuntimeException {
        if (startIndex < 0) {
